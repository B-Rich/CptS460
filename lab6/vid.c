...
int scroll()
{
    ...
    //before we start scrolling lets clear out the clock so it doesnt scroll too
    //w = 0x0700;

    //for (i=25*80-25; i<25*80; i++){                               
      //      put_word(w, base, 0+2*i);        // write 24*80 blanks to VGA memory
       // }
...
//This sets the clock in the corner to the time specified in seconds
int setClock(i) int i;
{
    int pos,w,offset,seconds,minutes,hours,s1,s0,m1,m0,h1,h0;
    ...
    //set the 0th collumn of seconds (1s)
    pos = 2*(24*80 + 79);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + s0+48;
    ...
    put_word(w, base, offset);
    //set the 1st collumn of seconds (10s)
    pos = 2*(24*80 + 78);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + s1+48;
    ...
    //set the middle of the clock to :
    pos = 2*(24*80 + 77);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + ':';
    ...
    //set the 0th collumn of minutes (1s)
    pos = 2*(24*80 + 76);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + m0+48;
    ...
    put_word(w, base, offset);
    //set the 1st collumn of minutes (10s)
    pos = 2*(24*80 + 75);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + m1+48;
 ...
 //set the 0th collumn of hours (1s)
    pos = 2*(24*80 + 73);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + h0+48;
    ...
    put_word(w, base, offset);
    //set the 1st collumn of hours (10s)
    pos = 2*(24*80 + 72);
    offset = (org + pos) & vid_mask;
    w = color;
    w = (w << 8) + h1+48;
